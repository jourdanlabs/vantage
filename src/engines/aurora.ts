...
export async function runAURORA(
  ...
  // Ship gate uses the **security** surface only (Kaioken surface split).
  // Quality findings stay in the report but do not block APPROVED.
  let adversarialPenalty = 0;
  for (const f of pulsar.adversarialFindings) {
    if (f.surface === 'quality') continue;
  ...
  // High severity security findings only (quality surface does not block ship)
  for (const finding of pulsar.adversarialFindings
    .filter((f) => f.severity === 'HIGH' && f.surface !== 'quality')
    .slice(0, 5)) {
    issues.push({
      severity: 'HIGH',
      file: finding.file,
  ...
  const summaryParts = [
    ...
    `${eclipse.highRisk.length} high-risk files identified. ${
      pulsar.adversarialFindings.filter((f) => f.severity === 'HIGH' && f.surface !== 'quality')
        .length
    } critical security findings (${
      pulsar.adversarialFindings.filter((f) => f.surface === 'quality').length
    } quality notes).`,
    verdict === 'APPROVED'
