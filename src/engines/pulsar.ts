...
function findMissingErrorBoundaries(content: string, filePath: string, language: string): AdversarialFinding[] {
  ...
  if (language === 'typescript' || language === 'javascript') {
    // JSON.parse without try/catch — reliability / error-boundary signal.
    // Under --semantic, the Kaioken merge in engines/index.ts drops these so
    // BenchProctor deserial safe-twins (JSON.parse) don't flood FPR; pattern
    // mode keeps them for corpora whose GT includes error-boundary (Juice Shop).
    for (let i = 0; i < lines.length; i++) {
...
function findHardcodedSecrets(content: string, filePath: string): AdversarialFinding[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (secretAssign) {
      ...
      const lhs = secretAssign[0] || '';
      // Skip obvious placeholders, test values, and non-credential tokens
      // (UUIDs, provider labels, UID/scope/storage-key constants, cookie headers,
      // template/expression strings that assemble runtime secrets rather than embed them)
      if (
        !/placeholder|changeme|your_key|example|XXXXXX|insert_|test_|<YOUR|TODO|FIXME|default_config_label/i.test(value) &&
        !/\(UUID:|providerToken|UuidFactory|^[0-9a-f]{8}-[0-9a-f]{4}-/i.test(value) &&
        !/^https?:\/\//i.test(value) &&
        // Strapi/CMS style UIDs and OAuth scope strings — not secrets
        !/::/.test(value) &&
        !/^[a-z0-9_.:-]+:[a-z0-9_.:-]+$/i.test(value) &&
        // Identifier names that are keys/scopes/uids, not credential stores
        !/(STORAGE_KEY|_UID\b|_SCOPE\b|TOKEN_UID|TOKEN_SCOPE)/i.test(lhs) &&
        // Cookie header construction (Set-Cookie strings), not embedded API keys
        !/Max-Age=|HttpOnly|SameSite=/i.test(value) &&
        // Template/expression assembly (`${id}:${secret}`, encode(cred)) — not a literal secret
        !/\$\{/.test(value) &&
        !/\bencode\s*\(/i.test(value)
      ) {
        findings.push({
          file: filePath,
          line: i + 1,
