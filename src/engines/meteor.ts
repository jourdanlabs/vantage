...
import {
  ...
  isKnownBasename, getLanguageForPath
} from '../languages';
...
function walkDir(dir: string, rootDir: string, ignorePatterns: string[]): string[] {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      if (SUPPORTED_EXTENSIONS.has(ext) || isKnownBasename(entry.name)) {
...
export async function runMETEOR(targetPath: string, onProgress?: (msg: string) => void): Promise<MeteorOutput> {
  ...
  for (const filePath of filePaths) {
    ...
    const langDef = getLanguageForPath(filePath);
    const language = langDef?.name ?? getLanguageName(ext);
    ...
    // Path-based lookup covers extensionless Dockerfiles.
    if (langDef?.fullySupported === false || !isLanguageFullySupported(ext)) {
